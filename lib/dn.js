...
DN.prototype.childOf = function (dn, caseInsensitive) {
  ...
  for (var i = dn.rdns.length - 1; i >= 0; i--) {
    ...
    for (var j = 0; j < keys.length; j++) {
      var k = keys[j];
        ...
        if (caseInsensitive) {
          if (ourRdn[k].toLowerCase() !== rdn[k].toLowerCase())
            return false;
        } else {
          if (ourRdn[k] !== rdn[k])
            return false;
...
DN.prototype.parentOf = function (dn, caseInsensitive) {
  if (caseInsensitive === undefined)
    caseInsensitive = false;
  if (typeof (dn) !== 'object')
    dn = parse(dn);
  ...
  for (var i = this.rdns.length - 1; i >= 0; i--) {
    ...
    for (var j = 0; j < keys.length; j++) {
      ...
      if (caseInsensitive) {
        if (theirRdn[k].toLowerCase() !== rdn[k].toLowerCase())
          return false;
      } else {
        if (theirRdn[k] !== rdn[k])
          return false;
...
DN.prototype.equals = function (dn, caseInsensitive) {
  if (caseInsensitive === undefined)
    caseInsensitive = false;
  if (typeof (dn) !== 'object')
    dn = parse(dn);
  ...
  for (var i = 0; i < this.rdns.length; i++) {
    ...
    for (var j = 0; j < ourKeys.length; j++) {
      if (caseInsensitive) {
        if (ourKeys[j].toLowerCase() !== theirKeys[j].toLowerCase())
          return false;
        if (ours[ourKeys[j]].toLowerCase() !== theirs[ourKeys[j]].toLowerCase())
          return false;
      } else {

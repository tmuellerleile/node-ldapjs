...
Server.prototype._getHandlerChain = function _getHandlerChain(req, res) {
  ...
  } else if (req.protocolOp === Protocol.LDAP_REQ_UNBIND) { // check unbind
    var unbindRoute;
    for (var r = 0; r < routes.length; r++) {
      if (routes[r].name === 'unbind') {
        unbindRoute = routes[r];
        break;
      }
    }
    return {
      backend: unbindRoute ? unbindRoute.backend : self,
      handlers: function getUnbindChain() {
        if (unbindRoute && unbindRoute[op])
          return unbindRoute[op];
  ...
  for (var i = 0; i < routes.length; i++) {
    ...
    // Special cases are abandons and exops, handle those first.
    if (req.protocolOp === Protocol.LDAP_REQ_EXTENSION) {
      if (route.name === req.requestName) {
        return {

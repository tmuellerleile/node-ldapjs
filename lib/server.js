...
function Server(options) {
  if (options) {
    ...
    if (options.caseInsensitiveRouting &&
        typeof (options.caseInsensitiveRouting) !== 'boolean')
      throw new TypeError('options.caseInsensitiveRouting must be a boolean value');
  ...
  this.caseInsensitiveRouting = (options.caseInsensitiveRouting !== undefined) ?
    options.caseInsensitiveRouting : false;
...
Server.prototype._getHandlerChain = function _getHandlerChain(req, res) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (route.dn.equals(req.dn, self.caseInsensitiveRouting) ||
        route.dn.parentOf(req.dn, self.caseInsensitiveRouting) ||
        suffix === '') {
      if (route[op]) {
